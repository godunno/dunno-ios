...
- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType
{
  ...
  switch (animationType) {
    ...
    case MJPopupViewAnimationSlideBellowNavbarTop:
    case MJPopupViewAnimationSlideBellowNavbarBottom:
...
- (void)slideViewIn:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView withAnimationType:(MJPopupViewAnimation)animationType
{
  ...
  //default popupEndRect
  CGRect popupEndRect = CGRectMake((sourceSize.width - popupSize.width) / 2,
                                   (sourceSize.height - popupSize.height) / 2,
                                   popupSize.width,
                                   popupSize.height);

  //**BellowNavbar** types aways comes from top
  if(animationType==MJPopupViewAnimationSlideBellowNavbarBottom||animationType==MJPopupViewAnimationSlideBellowNavbarTop){
    popupEndRect = CGRectMake((sourceSize.width - popupSize.width) / 2,
                              (self.navigationController.navigationBar.frame.size.height + [UIApplication sharedApplication].statusBarFrame.size.height) + 10,
                                     popupSize.width,
                                     popupSize.height);
  }
